...
$(document).ready(function () {
    // Fetch meals
    function fetchMeals(page = 1, search = '', type = '') {
        $.ajax({
            url: 'ajax.php',
            type: 'POST',
            data: {
            ...
            success: function (response) {
                ...
                if (data.success) {
                    $('#mealsContainer').html(data.mealsHtml);
                } else {
    ...
    $(document).on('click', '.selectMealBtn', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                const data = JSON.parse(response);
                if (data.success) {
                    // Show modal with meal info
                    $('#mealName').text(`Meal: ${mealName}`);
                    $('#mealQuantity').val(10); // Default quantity
            ...
            error: function () {
                console.error('Failed to select meal.');
            },
        });
    });
...
function validId(val) {
    if (val ==1){
        ...
        document.getElementById('id_card_no').setAttribute('data-error',"Enter 10 Character(include '/') Valid Voter ID Card No");
    }else if(val ==3){
        document.getElementById('id_card_no').setAttribute('type','text');
        document.getElementById('id_card_no').setAttribute('data-minlength','10');
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.done == true) {
                ...
                $('#getLaundryCost_n').html(response.laundry_cost);

                // Calculate total amount to be paid (remaining price + laundry cost)
                var totalAmount = parseFloat(response.remaining_price) + parseFloat(response.laundry_cost);
                $('#remaining_amount').attr('placeholder', 'Enter ' + totalAmount + ' to check out');
                $('#totalAmount').val(totalAmount); // Store the total amount in a hidden field
...
$('#checkOutRoom_n').submit(function () {
    ...
    // Ensure that the amounts are correctly parsed and compared
    if (remaining_amount == totalAmount) {
        alert('The entered amount must match the total amount due.');
        return false;  // Prevent form submission if amounts do not match
    }
...
$('#openSelectQuantityModalButton').on('click', function () {
    ...
    $('#selectQuantityModal').modal('show');
});

// Close Select Quantity Modal and restore body scroll
$('#selectQuantityModal').on('hidden.bs.modal', function () {
    // Remove modal-open class to re-enable scrolling on the page
    $('body').removeClass('modal-open');
    // Optional: you can also hide the backdrop if using Bootstrap modal behavior
